...
int* dividepivo(int* vet, int n, int mom){
     ...
     if(contmaior>contmenor){
       ...
       vetpoint[1][contmaior+2]=mom;    
       return vetpoint[1];         
     }else{
       vetpoint[0];
       vetpoint[0][contmenor+1]=-1;
       vetpoint[0][contmenor+2]=mom;    
       return vetpoint[0];  
     }     
} 
...
int ncomp=(n/2)+2;
printf(" o numero de termos é :%d\n",ncomp);
int n2=calculatamanho(vet);
printf(" o numero de termos no novo vetor é:%d\n",n2);
if(n2>ncomp){
  selecionamediana(vet,n2);
}else{
  printf("a mediana das medianasa é :%d\n",vet[n2+1]);
 } 
}
